...
class KubernetesInterface(object):
    ...
    def _get(self, api, namespace=None):
        try:
            ...
            logger.error('Error while calling  %s:%s', api, e)
            raise e
        if response.status_code == 200:
            success = True
        elif response.status_code == 401:
            logger.error("Request to api server is not authorized")
            response.raise_for_status()
        elif response.status_code == 404:
            logger.info("Resource not found: %s namespace %s)", api, namespace)
            success = False
        else:
...
#       self.watch_all_routes.start()
    ...
    def _events(self, api, resource_version):
        ...
        try:
            ...
            logger.warning("Watching connection raised an exception: %s" %re)
        ...
        try:
            if sb_param_name in object['metadata']['annotations']:
                ...
                if isinstance(sb_param, dict):
                    ...
                    logger.error("Additional stylebook parameters are not dictionary for the resource %s.%s", object['metadata']['name'], object['metadata']['namespace'])
        except ValueError as e:
            logger.error("Stylebook Additional Params for the resource %s.%s are not decoded in proper json: %s", object['metadata']['name'], object['metadata']['namespace'], e)
        return {}
    ...
    def get_ingress(self, ingress):
        ...
        if 'annotations' in ingress['metadata']:
            ...
            ing['app_ports'][0]['app_port']=int(ingress['metadata']['annotations'].get('NETSCALER_INSECURE_PORT', 80))
            ing['app_ports'][0]['protocol']=ingress['metadata']['annotations'].get('NETSCALER_INSECURE_SERVICETYPE', "http").lower()
        ...
        if 'rules' in ingress['spec']:
            ...
            for rule in ingress['spec']['rules']:
                ...
                if 'host' in rule:
                    ...
                    host_expr = 'HTTP.REQ.HOSTNAME.SERVER.EQ("' + rule['host'] + '")'
...
#        self.delete_certkey(secret)
    ...
    def configure_cpx_for_all_apps(self):
        try:
            ...
            logger.error("Problem configuring target devices: %s", e)
            self.reinit_kubernetes = True
            return
        ...
        for ep in event['subsets']:
            if 'addresses' in ep:
                for port in ep['ports']:
                    ...
                    lbapp = self.netskaler.nslbapps.get(lbname)
                    if lbapp == None:
            ...
            for app_port in ing['app_ports']:
                ...
                if cs_name in self.netskaler.nsapps:
                    ...
                    for _, pol in cs_app.policies.iteritems():
                        ...
                        if lb_app != None:
                            lb_app.nsapps.remove(cs_app)
                    # Managing pending services list.
                    # Once ingress is removed, it should be removed from the servces that it was waiting for
                    del remove_svc[:]
                    ...
                    # Managing pending secrets list.
                    # Once ingress is removed, it should be removed from the secrets that it was waiting for
                    # reusing svc
                    del remove_svc[:]
                    for key, svc in self.pending_secrets.iteritems():
                        if cs_app in svc:
                            svc.remove(cs_app)
                        if len(svc) == 0:
                            # The last waiting ingress is removed from the service's list
    ...
    def  _fill_service(self, app):
        ...
        if 'clusterIP' not in app['spec'] or app['spec']['clusterIP'].lower() == 'none':
             app['spec']['clusterIP'] = None
