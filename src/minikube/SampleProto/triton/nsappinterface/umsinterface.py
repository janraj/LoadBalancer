            ...
            for ns_d in ns_devices:
                if ns_d['task_id'] == taskid:
                    ...
                    try:
                        ...
                        logger.error('DELETE Netscaler: Error while calling %s: %s', url, e.message)
                    return
        except requests.exceptions.RequestException as e:
            logger.error('Get All NS Devices: Error while calling %s: %s', url, e.message)
            return
...
class AppmanagerInterface(UMSInterface):
    ...
    def create(self, nsapp, app_payload):
        try:
            ...
            logger.error('Create Application: Error while calling %s: %s', url, e.message)
            return
        try:
            ...
            logger.error('Create Application: Error accessing device id in Appmanager payload %s', e.message)
    ...
    def delete(self, nsapp):
        try:
            ...
            logger.error('DELETE Application: Error while calling %s: %s', url, e.message)
            return
    ...
    def sync_ns_apps(self):
        try:
            ...
            logger.error('Get All Apps: Application: Error while calling %s: %s', url, e.message)
            return
        try:
